// MakerBay visual E2E — Part B3: Southside demo grounded Q&A + full booking.
const fs = require('fs');
const path = require('path');
const DIR = __dirname;
const SHOTS = path.join(DIR, '.shots');
const LOG = path.join(SHOTS, 'partB3-log.json');
...
(async () => {
  try {
    // 1. Open the demo page, mount the widget, capture its URL
    await nav('demo', 'https://demo.makerbay.app', 5000);
    await ev('demo:open-widget', clickByText('/ask a question/i'));
    await sleep(3000);
    const src = await ev('demo:iframe-src', "(()=>{const f=[...document.querySelectorAll('iframe')].map(i=>i.src).find(s=>/chat\\.|embed/.test(s)); return f||'none'})()");
    if (!src || src === 'none') throw new Error('widget iframe URL not found');

    // 2. Grounded Q&A on Southside's assistant
    await nav('ss-chat', src, 5000);
    await shot('ss-0-open.jpg');
    ...
    await sleep(3000);
    await shot('ss-2-days.jpg');
    await ev('ss:day', clickByText('/friday 11 september/i'));
    await sleep(2500);
    await shot('ss-3-times.jpg');
    ...
    await sleep(500);
    await shot('ss-5-filled.jpg');
    await ev('ss:confirm', clickByText('/confirm|request|book|done/i'));
    await sleep(4500);
    await shot('ss-6-confirmation.jpg');
    await ev('ss:final-text', "document.body.innerText.slice(0,400)");
