...
### 131 — Platform Owner settings in the Admin Portal ✅ built, awaiting deploy

**Shipped 2026-08-29.** Approved: one constant, read-only admin page, footer
rewritten to name both parties and the relationship.

- `packages/email/src/platform.ts` holds the identity: Appa Technologies Pty
  Ltd, 4 Greenwood Place, Freshwater NSW 2096, and support@makerbay.app. A
  constant rather than a setting, because the Cognito user pool renders
  `authEmail('verify').html` at CDK synth and stores it in the CloudFormation
  resource - no runtime lookup can reach that copy, so a database-backed value
  would leave one email of 22 permanently disagreeing with the rest. Verified
  in the synth output: the user pool now carries the new entity and address.
- The customer footer names the business, MakerBay, and the relationship
  between them, which is what CASL requires when sending on someone else's
  behalf. The old line put an address the tradesperson does not occupy directly
  under their own phone number.
- Read-only admin page at `/platform`, rendering the footers by calling the
  real functions, so the screen cannot drift from the inbox.
- Tests rewritten to assert the rule rather than the placeholder, plus one that
  fails if `MakerBay Pty Ltd` or `Wilson Street` ever returns.

**Found and fixed along the way:**
- The admin API registered `[GET, POST]` while the handler served `DELETE`
  and the console called it, so "remove from the suppression list" was a dead
  button with nothing in the Lambda logs. CORS omitted DELETE too, so the
  preflight failed first. Now `ANY` plus DELETE in CORS: two route resources
  become one (492 of 500) and the mismatch cannot recur.
- `renderEmail` never wrapped footer lines. Latent while the longest was 67
  characters against a 72 limit; the new address is 102. Wrapped, except lines
  carrying a URL, which must stay joined to be copyable.
- `logoUrl` and `footerNote` were computed on every send by
  `getTenantBrand` and read by nothing. `footerNote` had gone further than
  dead: it said "Sent by X via MakerBay", which now contradicts the real
  footer. Both removed.
- `.muted` was used on every admin page and defined in no stylesheet.

**Deferred deliberately:** making it editable needs the staff role system
first. `staffRole` is plumbed through the authorizer and checked in zero
routes, so today every staff member can suspend tenants and grant
...
### 131b (superseded heading, original brief below)
...
### 132 — Email coverage audit: one path bypasses sendEmail entirely ✅ built, awaiting deploy

**Shipped 2026-08-29.**
- The ticket reply now renders `owner.ticketReply()` and goes through
  `sendEmail` with `ref: { refType: 'ticket' }`, so it gets the suppression
  check, the delivery tracking and the footer. It was the one email a business
  owner reads while already unhappy with us, and it was the least cared-for in
  the system.
- `packages/core/src/senders.test.ts` reads the source tree and fails if any
  file outside the allowlist constructs a `SendEmailCommand`. The allowlist
  carries counts, not just names, so a second call in an already-listed file is
  caught. Verified by introducing a violation and watching it fail.
- The self-only SES diagnostic stays raw and is documented in the allowlist:
  routing it through `sendEmail` would test `sendEmail` rather than SES,
  which is the one thing it must not do.
- Gallery regenerated by `scripts/email-gallery.mts`, driven off the template
  exports with a coverage check that fails when a template is added and not
  listed. 24 versions of 20 templates, including the branches where wording
  changes (invoice already paid, booking with a deposit, withheld number).

### 132a (superseded heading, original brief below)
...
Written plain, for a plumber rather than a lawyer, and grounded in what the
system actually does rather than boilerplate: no cookies anywhere on the site,
no third-party analytics, exactly two subprocessors (AWS and Stripe), Bedrock
inference inside our own account with no training on customer content, and the
controller/processor split spelled out because it decides who a homeowner asks
for deletion.

**Blocking on the founder:**
1. The ABN is marked [TO BE INSERTED] in the Terms.
2. Data is stored in us-east-1. Stated prominently rather than buried, because
   it is the fact most likely to matter to a UK or EU customer. APP 8 requires
   the disclosure; UK/EU need a transfer mechanism, and the draft relies on the
   standard contractual clauses in the AWS agreement. Worth confirming that is
   the position we want before signing up EU businesses.
3. No Data Processing Agreement is offered. UK and EU business customers will
   ask for one, and it is a separate document.
4. This is a careful draft, not legal advice, and it has not been reviewed by a
